...
monie_line_plot(p_est, cluster_edges, '--', "orange")
...
monie_line_plot(p_est, cluster_edges, '-.', "pink")

custom_legend = [
    Line2D([0], [0], color='orange', linestyle='--', linewidth=2, label='Frequency'),
    Line2D([0], [0], color=deep_blue, linestyle='-', linewidth=2, label='kmeans'),
    Line2D([0], [0], color='green', linestyle=':', linewidth=2, label='uniform'),
    Line2D([0], [0], color='pink', linestyle='-.', linewidth=2, label='log')
]
plt.legend(handles=custom_legend, loc='upper right')
plt.savefig("plots/hours_played/cluster_visualization_3")
plt.show()

...
class Args:
    bin: int = 8 
    # TODO: find the best group size from KNN classification
...
def run_sklearn_TREE(feature_subset: pd.DataFrame, target: pd.Series, cont_mask, crit):
    ...
    '''
    metric_stack = []
    # filename = f"_log\\_log_TREE\\F={feature_name_list[i]}_T={target_name_list[j]}_GROUP={args.bin}_CRIT={crit}"
    filename = f"./_log/_recommend_textblob/F={feature_name_list[i]}_CRIT={crit}"
    print(filename)
...
def predict_hours_played():
    ...
    # TODO: perform chi_test on kmeans discretization
    ...
    target_freq_disc = pd.qcut(targets, q=args.bin, labels=np.arange(args.bin)).to_numpy()

    # Disc by equal range
    target_range_disc = pd.cut(targets, bins=args.bin, labels=np.arange(args.bin)).to_numpy()

    # Disc by kmeans clustering
    est = KBinsDiscretizer(n_bins=args.bin, encode="ordinal", strategy='kmeans')
    ...
    target_log_range_disc = pd.cut(targets, bins=args.bin, labels=np.arange(args.bin)).to_numpy()

    target_list = [target_kmeans_disc, ]
                   #target_range_disc, target_freq_disc, target_log_range_disc]
    target_name_list = ['kmeans', ]
                        #'range', 'freq', 'log_range']
